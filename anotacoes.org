...
* Apresentação Deise
- Principais tipos de Acesso Aberto: Essa classificação é para periódicos?
- Pq alguem com mais capital cientifico teria alguma aversão ao acesso aberto?
- Como se medir os diferentes níveis de capital?
- Não foi averiguada a área dos artigos especificamente, certo? Contanto que o pesquisador esteja vinculado a PosGrad de CB, toda sua produção entra, certo?
- Quanto menor o capital, maior a publicação em acesso aberto sem APC. Faz sentido!
- Quanto maior o capital, menor a publicaçãoem acesso aberto. Entretanto, a proporção de acesso aberto pago é mto maior
- Colocar o título, e numero das figuras como está na tese.

* Baixando dados:
- Problema: WoS costuma dar problemas (nao da pra baixar) quando tem muitos records com mta informaçao (mtos nomes de autores, p ex.)
- Isso acontece com o filtro padrao (relevance), que naturalmente deixa esses dados para o fim
- Usando outro filtro (e.g. 'oldest first') costuma amenizar o problema, mesmo que de vez em quando tenha que baixar menos records.

* Limpeza de dados:
** Scopus
- Consegui baixar os dados separando por area. Mas nao estava conseguindo remover duplicatas, pois o bibliometrics adiciona identificadores unicos no campo *SR*
- Logo, para remover as duplicatas precisa dsconsiderar esse campo. Ao fazer isso, o numero de records supostamente 'unicos' fica inclusive maior do que o de resultados na pesquisa do WoS do dia 31.
** Lens
- O numero de records (sem usar bibliometrix) e exatamente o mesmo de quando eu pesquisei no site.
- Ha um numero consideravel de duplicatas por titulo. Curioso que eles possuem ate mesmo identificadores diferentes. Olhar isso depois
** Dimensions
- O numero de records e o mesmo da pesquisa tambem. E tambem tem algumas entradas com titulo identico, provavelmente duplicatas.
- Entretanto, a funçao 'convert2df' do bibliometrix nao esta funcionando... Talvez precise limpar os dados?
** WoS
- Faltam alguns records (por volta de 300) ao se exportar os csv's.
